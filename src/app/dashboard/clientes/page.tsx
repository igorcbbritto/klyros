...
import { mockCustomers, mockTickets } from "@/dashboard-mocks";
...
export default function ClientesPage() {
  ...
  const customerTickets = (customerId: string) =>
    mockTickets.filter((t) => t.customer_id === customerId);
  ...
  return (
    <DashboardLayout>
      ...
      {/* New Customer Modal */}
      <Modal isOpen={showNewCustomer} onClose={() => setShowNewCustomer(false)} title="Novo Cliente">
        <form onSubmit={(e) => { e.preventDefault(); setShowNewCustomer(false); }} className="space-y-4">
          <div>
          ...
          <div>
            ...
            <input type="tel" className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="(00) 00000-0000" />
          </div>
          <div>
          ...
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={() => setShowNewCustomer(false)} className="px-4 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded-lg transition-colors">Cancelar</button>
          ...
          <div className="space-y-4">
            <div className="space-y-3 text-sm">
              ...
              {selectedCustomer.notes && (
                <p className="text-gray-500 text-sm italic mt-2">{selectedCustomer.notes}</p>
              )}
            </div>
            <div>
              <h4 className="font-medium text-gray-900 mb-2">Chamados Recentes</h4>
